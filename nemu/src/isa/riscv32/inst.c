...
//word_t默认为uint32
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli, I, printf("slli %0d\n", imm); R(rd) = src1 << imm);
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(rd) = (uint32_t)src1 < (uint32_t)imm ? 1 : 0);//无符号比较立即数
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai, I,  imm = BITS(imm, 4, 0); printf("srai%0d\n", imm); R(rd) = (SEXT(BITS(src1, 31, 31), 1) << (32 - imm)) | (src1 >> imm)); // 算数右移
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli, I, printf("srli%0d\n", imm); R(rd) = src1 >> imm);                                                                         // 逻辑右移
  // B型指令

...
//qpi
    ...
    PSRAM_INIT_QPI QPI(
        ...
        .ce_n(qpi_ce_n),
        .dout(qpi_dout),
        .douten(qpi_doe) ////数据输出使能（1=驱动数据线)
    );

    assign qpi_req  =state==ST_INIT;
//wb_we上层发出的，先检查初始化信号
    assign sck      =(state==ST_INIT)?qpi_sck : wb_we ? mw_sck  : mr_sck;
    assign ce_n     =(state==ST_INIT)?qpi_ce_n : wb_we ? mw_ce_n : mr_ce_n;
    assign dout     =(state==ST_INIT)?qpi_dout : wb_we ? mw_dout : mr_dout;
    assign douten   =(state==ST_INIT)?{4{qpi_doe}}: wb_we ? {4{mw_doe}}  : {4{mr_doe}};
